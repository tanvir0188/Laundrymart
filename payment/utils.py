...
def create_stripe_customer(user):
  ...
  if not user.order_set.filter(stripe_customer_id__isnull=False).exists():
  ...
  return user.order_set.exclude(stripe_customer_id__isnull=True).first().stripe_customer_id
